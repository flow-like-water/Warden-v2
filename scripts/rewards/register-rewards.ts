...
async function main() {
  ...
  const OldWarden = new ethers.Contract(
    OLD_WARDEN_ADDRESS,
    OldWardenABI,
    provider
  );

  const Boostv2 = new ethers.Contract(
    DELEGATION_BOOST_ADDRESS,
    veBoostv2ABI,
    provider
  );
  ...
  const old_registers = await OldWarden.queryFilter(
    OldWarden.filters.Registred(),
    13992370,
    old_warden_check_block
  );
  for (let r of old_registers) {
    const user_addr = r.args["user"];

    const still_registered = !(
      await OldWarden.userIndex(user_addr, { blockTag: old_warden_check_block })
    ).eq(0);

    if (still_registered && !old_registered_users.includes(user_addr))
      old_registered_users.push(user_addr);
  }

  // Get all v2 Registered users (events)
  const registers = await Warden.queryFilter(
    Warden.filters.Registred(),
    start_block,
    end_block
  );
  ...
  for (let r of registers) {
    ...
    const quitted = (
      await Warden.userIndex(user_addr, { blockTag: end_block })
    ).eq(0);
    if (quitted) continue;
    ...
    const end_block_balance = await veCRV.balanceOf(user_addr, {
      blockTag: end_block,
    });
    const offer_index = await Warden.userIndex(user_addr, {
      blockTag: end_block,
    });
    const offer_params = await Warden.offers(offer_index, {
      blockTag: end_block,
    });
    const registered_balance = end_block_balance
      .mul(offer_params.maxPerc)
      .div(10000);
    registration.points = registration.points.add(
      registered_balance.mul(10000).div(UNIT)
    );
    ...
    const delegating_when_registering = await Boostv2.delegated_balance(
      user_addr,
      { blockTag: event_blocknumber }
    );
    const register_block_balance = await veCRV.balanceOf(user_addr, {
      blockTag: event_blocknumber,
    });
    const register_offer_index = await Warden.userIndex(user_addr, {
      blockTag: event_blocknumber,
    });
    const registering_balance = register_block_balance
      .mul(
        (
          await Warden.offers(register_offer_index, {
            blockTag: event_blocknumber,
          })
        ).maxPerc
      )
    ...
    if (delegating_when_registering.gt(blocked_balance)) {
      ...
      const extra_ratio = BigNumber.from(10000).sub(
        extra_delegated.mul(10000).div(register_block_balance)
      );
      registration.points = registration.points
        .mul(BigNumber.from(10000).sub(extra_ratio))
        .div(10000);
    }
    ...
    if (old_registered_users.includes(user_addr)) {
      registration.points = registration.points.add(
        registration.points.div(10)
      );
    }

    // punishement if not Registered anymore 2 weeks after end of rewards period (slashed 50%)
    const quitted_punishement_block = (
      await Warden.userIndex(user_addr, { blockTag: punishement_end_block })
    ).eq(0);
  ...
  for (let u of rewarded_users) {
    ...
    let user_registration: RegisteredUser = user_registrations.get(
      u
    ) as RegisteredUser;
    ...
    console.log(
      u,
      "- Points:",
      user_registration.points.toString().padStart(12),
      "- Rewards:",
      displayBalance(expected_rewards).padStart(12)
    );
